...
CDesign::CDesign(_ConnectionPtr  m_pConnection):CBase(m_pConnection)
{
...
BOOL CDesign::Show(CEdit* pEdit,int nIndex,CString str)
{
	CString strSQL;
	strSQL.Format("select * from Design where fnum='%d'",nIndex);
...
BOOL CDesign::List(CListBox* pList)
{
	CString strSQL;
...
BOOL CDesign::Delete(int nIndex)
{
	CString strSQL;
	strSQL.Format("delete from Design where fnum='%d';",nIndex); 
	try
	{
...
BOOL CDesign::SetPcon(_ConnectionPtr  m_pCon)
{
	m_pConnection = m_pCon;
	return TRUE;
}

BOOL CDesign::Input(CEdit* pEdit,int nIndex,CString str)
{   
	CString strtext;
	...
	CString strSQL;
	strSQL.Format("update Design set %s = '%s' where fnum = '%d'",str,strtext,nIndex);
...
BOOL CDesign::CreateNew(int p)
{
   	CString strSQL;
	strSQL.Format("insert into Design(fnum,ftitle,fcontent,fremark,fanswer) \
...
void CDesign::Check(int p)
{
	...
	CString strSQL;
	strSQL.Format("select * from design where fnum='%d'",p);
												//指定题号的将被检测的试题
	try
	{
		m_pRecordset.CreateInstance(__uuidof(Recordset));
...
BOOL CDesign::CheckOver(int n)
{
	...
	CString strSQL;
	strSQL.Format("select * from design where fnum='%d'",n);
	CString strOut;
